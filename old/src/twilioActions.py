...
def send_confirmation(id: int, user_type: UserType, is_admin: bool, msg: str):
    ...
    if id < 1:  # if id < 1, do nothing. supervisor id can be < 1 if it's for an employee with no supervisor
        return False
...
def confirm_submission(employee_id: int, msg: str, fro: str):
    ...
    # admin id is 1 and my employee id is 2. so in either of those cases, we don't want to send confirmations
    is_admin = employee_id < 3
